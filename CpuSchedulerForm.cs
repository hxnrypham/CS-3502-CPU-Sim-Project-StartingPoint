...
4. Compare algorithm behaviors and outcomes";
        ...
        private List<SchedulingResult> RunHRRNAlgorithm(List<ProcessData> processes)
        {
            ...
            int currentTime = remaining.Min(p => p.ArrivalTime);
            ...
            while (remaining.Count > 0)
            {
                ...
                var chosen = available
                    .Select(p => new { P = p, R = ((currentTime - p.ArrivalTime) + p.BurstTime) / (double)Math.Max(1, p.BurstTime) })
                    .OrderByDescending(x => x.R)
                    .ThenBy(x => x.P.ArrivalTime)
                    .First().P;

                int start = Math.Max(currentTime, chosen.ArrivalTime);
                int finish = start + chosen.BurstTime;
                ...
                results.Add(new SchedulingResult
                {
                    ...
                    StartTime = start,
                    FinishTime = finish,
                    WaitingTime = start - chosen.ArrivalTime,
                    TurnaroundTime = finish - chosen.ArrivalTime
                });
        ...
        // Lottery Scheduling (non-preemptive). Uses Priority as ticket count (min 1).
        private List<SchedulingResult> RunLotteryAlgorithm(List<ProcessData> processes)
        {
            ...
            while (remaining.Count > 0)
            {
                ...
                int totalTickets = available.Sum(p => Math.Max(1, p.Priority));
                int draw = random.Next(1, totalTickets + 1); // reuse form's Random
                int cumulative = 0;
                ProcessData chosen = null;
                foreach (var p in available)
                {
                    cumulative += Math.Max(1, p.Priority);
                    if (draw <= cumulative) { chosen = p; break; }
                }
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            int earliestArrival = results.Min(r => r.ArrivalTime);
            int lastFinish = results.Max(r => r.FinishTime);
            int totalTime = Math.Max(0, lastFinish - earliestArrival); // includes idle time
            int totalBurst = results.Sum(r => r.BurstTime);
            double cpuUtilization = totalTime > 0 ? (totalBurst / (double)totalTime) * 100.0 : 0.0;
            double throughput = totalTime > 0 ? results.Count / (double)totalTime : 0.0;
            ...
            summaryItem.SubItems.Add($"Thru: {throughput:F3}/unit");
        ...
        private void btnHRRN_Click(object sender, EventArgs e)
        {
            ...
            if (data.Count == 0) { MessageBox.Show("Please set process count and ensure the data grid has process data.", "No Process Data", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }

            var results = RunHRRNAlgorithm(data);
            DisplaySchedulingResults(results, "HRRN");
            ShowPanel(resultsPanel);
            sidePanel.Height = btnDashBoard.Height;
            sidePanel.Top = btnDashBoard.Top;
        }
        ...
        private void btnLottery_Click(object sender, EventArgs e)
        {
            ...
            var results = RunLotteryAlgorithm(data);
            DisplaySchedulingResults(results, "Lottery");
            ShowPanel(resultsPanel);
            sidePanel.Height = btnDashBoard.Height;
            sidePanel.Top = btnDashBoard.Top;
        }
